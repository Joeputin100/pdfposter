...
- **Skip-completed-bands resume** (Phase 2 review finding): the streamed PNG is one continuous zlib stream, so a partial file can't be appended — `upscaleToFile` re-encodes from band 0 on resume (a kill safely re-runs, never corrupts). True skip-resume needs a raw-rows scratch-prefix file (the spec's "persisted prefix" / Photoshop scratch-disk idea); the band-keyed `saveBand`/`lastBand` API + `resumeFromBand` param are already in place so it can be added without touching callers. The foreground service makes mid-run kills unlikely, so this is a UX optimization, not a correctness gap.
- **RegionSource whole-decode `inSampleSize`** (Phase 2 review finding): the non-region-decodable fallback decodes the source whole without `inSampleSize`, so a huge source in an exotic format could OOM on the *source* decode (the 4× output is already bounded by streaming). Common formats (JPEG/PNG/WebP) all region-decode, so this is rare; fold an `inSampleSize`-bounded source decode into the Task 8/9 memory-guard work.
