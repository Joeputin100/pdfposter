        ...
        return try {
            ...
            uploadTask.addOnProgressListener { snap ->
                if (snap.totalByteCount > 0) {
                    ...
                    onPhase(Phase.UPLOADING, 0.15f + 0.30f * frac)
                }
            }
            uploadTask.await()
            ...
            onPhase(Phase.IN_PROGRESS, 0.55f)
            val outputUrl = pollForCompletion(functions, txId, onPhase)

            // 5. Download.
            onPhase(Phase.DOWNLOADING, 0.85f)
            ...
            onPhase(Phase.SUCCEEDED, 1.0f)
            Result.success(outFile)
        } catch (t: Throwable) {
            Log.w(TAG, "ai upscale failed: ${t.message}", t)
            onPhase(Phase.FAILED, 0f)
            Result.failure(t)
        }
    }
    ...
    private suspend fun pollForCompletion(
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (status != lastStatus) {
                Log.i(TAG, "tx $txId status=$status")
                lastStatus = status
                when (status) {
                    "in_queue" -> onPhase(Phase.IN_QUEUE, 0.55f)
                    "in_progress" -> onPhase(Phase.IN_PROGRESS, 0.65f)
